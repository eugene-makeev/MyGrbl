...
#ifndef SILENT_MODE
    printPgmString(
            PSTR("[HLP:$$ $# $G $I $N $x=val $Nx=line $J=line $SLP $C $X $H ~ ! ? ctrl-x]\r\n"));
#endif
}
    ...
    for (set_idx = 0; set_idx < AXIS_N_SETTINGS; set_idx++)
    {
        for (idx = 0; idx < N_AXIS; idx++)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                report_util_float_setting(val + idx, settings.acceleration[idx] / (60 * 60),
                N_DECIMAL_SETTINGVALUE);
                break;
            case 3:
...
void report_gcode_modes()
{
    ...
    { // Note: Multiple coolant states may be active at the same time.
        ...
        {   report_util_gcode_modes_M(); serial_write('7');}
        if (gc_state.modal.coolant & PL_COND_FLAG_COOLANT_FLOOD)
        {   report_util_gcode_modes_M(); serial_write('8');}
    }
    else
...
void report_realtime_status()
{
    ...
    float wco[N_AXIS];
    if (bit_isfalse(settings.status_report_mask,
            BITFLAG_RT_STATUS_POSITION_TYPE) || (sys.report_wco_counter == 0))
    {
        for (idx = 0; idx < N_AXIS; idx++)
        {
